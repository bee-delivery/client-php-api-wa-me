...
class WhatsApp
{
    ...
    /**
     * Realiza uma solicitação HTTP utilizando a biblioteca cURL.
     *
     * Esta função é responsável por enviar requisições HTTP para um servidor remoto
     * usando a biblioteca cURL. Suporta métodos GET, POST e outros métodos personalizados,
     * permitindo também a inclusão de dados no corpo da requisição.
     *
     * @throws RuntimeException Se ocorrer um erro durante a execução da solicitação cURL.
     *
     * @return string Retorna a resposta do servidor após a solicitação bem-sucedida.
     */
    private function request()
    {
        // Adiciona o cabeçalho padrão para requisições JSON
        $this->header[] = 'Content-Type: application/json';

        // Configuração das opções cURL
        $options = array(
            CURLOPT_URL => $this->server . $this->parth,
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_CUSTOMREQUEST => $this->method,
            CURLOPT_HTTPHEADER => $this->header,
        );

        // Adiciona dados ao corpo da requisição, se existirem
        if (!empty($this->body)) {
            $options[CURLOPT_POSTFIELDS] = $this->body;
        }

        // Define o método como POST, se necessário
        if ($this->method === 'POST') {
            $options[CURLOPT_POST] = true;
        }

        // Inicializa uma sessão cURL e configura as opções
        $ch = curl_init();
        curl_setopt_array($ch, $options);

        // Executa a solicitação e obtém a resposta do servidor
        $result = curl_exec($ch);
        ...
        // Fecha a sessão cURL e retorna a resposta do servidor
        curl_close($ch);
        return $result;
    }
